...
@interface PagesScrollView () <UIScrollViewDelegate>
...
- (CGRect)frameForPageAtIndex:(NSUInteger)index {
    ...
    pageFrame.size.height=self.bounds.size.height;
    return pageFrame;
}
...
- (void)logSize:(CGSize)size withName:(NSString *)name {
    NSLog(@"%@: %f, %f",name,size.width,size.height);
}
- (void)logLayout {
    NSLog(@"#### PagingScrollView ###");
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    if ([self.pagingViewDelegate respondsToSelector:@selector(pagingScrollView:didScrollatIndex:)]) {
        [self.pagingViewDelegate pagingScrollView:self didScrollatIndex:_currentPagingIndex];
    }
}
