...
-(void) makeToastToBottom:(NSString *)message
{
    ...
    if (self.frame.origin.y != 0) {
       needCutH = (self.frame.size.height + self.frame.origin.y) - appRect.size.height;
    }else if(self.frame.origin.y == 0 && (self.frame.size.height -appRect.size.height) > toast.frame.size.height){
        needCutH = selfViewRect.size.height - appRect.size.height;
    }
    
    
    CGRect toastRec = toast.frame;
    CGPoint point = CGPointMake(appRect.size.width/2 - (toastRec.size.width/2), selfViewRect.size.height - (toastRec.size.height + needCutH));
    toast.frame = CGRectMake(point.x, point.y, toastRec.size.width, toastRec.size.height);
...
- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)point {
    ...
    toast.center = CGPointMake(mainSize.width/2, mainSize.height);
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - toast.frame.size.height) - CSToastVerticalPadding);
